...
const roadmapPath = path.resolve(__dirname, '../../../../brain/2a8c8dbd-4096-480c-b8e2-5469a35f82f0/future_roadmap.md');

async function publishRoadmap() {
  if (!fs.existsSync(roadmapPath)) {
    console.error(`Roadmap markdown file not found at: ${roadmapPath}`);
    process.exit(1);
  }

  console.log("Reading future_roadmap.md...");
  let markdownContent = fs.readFileSync(roadmapPath, 'utf-8');

  // Strip metadata lines if any exist (e.g. Created At/Completed At)
  ...
  // Convert GitHub alerts into stylized blockquotes for WordPress
  ...
  try {
    ...
    console.log("Checking if 'roadmap' page already exists on WordPress...");
    const checkRes = await fetch(`${WP_URL}/wp-json/wp/v2/pages?slug=roadmap`, {
    ...
    if (pages.length > 0) {
      ...
      console.log(`Page found with ID ${pageId}. Updating roadmap page...`);
      ...
      console.log("No existing roadmap page found. Creating a new page...");
    ...
    if (response.ok) {
      ...
      console.log(`\n✅ Success! The roadmap page is live at: ${data.link}`);
    } else {
