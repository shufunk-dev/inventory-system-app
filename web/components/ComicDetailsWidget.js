  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div className="space-y-6">
        <div className="bg-gray-800/30 p-6 rounded-2xl border border-gray-700/50 relative overflow-hidden group">
          ...
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">Publisher</h3>
          <p className="text-2xl font-black text-white capitalize">{item.comicPublisher || 'Unknown'}</p>
        </div>

        <div className="bg-gray-800/30 p-6 rounded-2xl border border-gray-700/50 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-sky-500/10 rounded-full blur-3xl -mr-10 -mt-10 transition-transform group-hover:scale-150"></div>
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">Issue Number</h3>
          <p className="text-2xl font-black text-white">{item.comicIssue || 'N/A'}</p>
        </div>
      </div>
        ...
        <div className="bg-gray-800/30 p-6 rounded-2xl border border-gray-700/50 h-full">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-gray-700/50 pb-4">
              ...
              <span className={`font-bold ${item.comicCondition && item.comicCondition.includes('.') ? 'text-sky-400' : 'text-white'}`}>
                {item.comicCondition || 'Raw / Ungraded'}
              </span>
            </div>
            
            <div className="flex justify-between items-center border-b border-gray-700/50 pb-4">
              <span className="text-gray-400">Grading Agency</span>
              <span className="font-bold text-white uppercase">{item.comicGradingAgency || 'N/A'}</span>
            </div>

            <div className="flex justify-between items-center">
              <span className="text-gray-400">Cert Number</span>
              <span className="font-mono text-white">{item.comicCertNumber || 'N/A'}</span>
            </div>
          </div>
        </div>
      </div>
